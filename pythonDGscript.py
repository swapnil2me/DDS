...
for j in states:
    for i in range(len(timeline)):
        ...
        p2.append(int(np.abs(300*rit(*(10,20))/20*(1-np.cos(i*np.pi/132.0/2.0))+vp)))
...
for j in states:
    for i in range(len(timeline)):
        if i<len(timeline)/1.9:
            ...
            vp = rit(*(rit(-15,-5),rit(-5,10)))
        p3.append(int(np.abs(300*rit(*(10,15))/15*(1-np.cos(i*np.pi/132.0/2.0))+vp)))
